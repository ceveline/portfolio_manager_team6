  ...
  try {
    ...
    if (!res.ok) {
      ...
      document.getElementById("ticker-modal-info").innerHTML =
        `<p style="color: var(--color-loss);">Unable to fetch data for ${ticker}</p>`;
    ...
    document.getElementById("ticker-modal-info").innerHTML =
      `<p style="color: var(--color-loss);">${errorMsg}</p>`;
    document.getElementById("ticker-modal-info").style.display = "block";
  }
}
...
function displayTickerInfo(data) {
  ...
  if (data.website) {
    websiteEl.innerHTML = `<a href="${data.website}" target="_blank" style="color: var(--color-accent); text-decoration: underline;">${data.website}</a>`;
  } else {
